             ...
             const vector<string>& genres, float rating)
{
    ...
    std::vector<string>::size_type size_dir = directors.size();
    for (std::vector<string>::size_type i = 0; i < size_dir; i++)
    {
        m_directors.push_back(directors[i]);
    }
    
    std::vector<string>::size_type size_act = actors.size();
    for (std::vector<string>::size_type i = 0; i < size_act; i++)
    {
        m_actors.push_back(actors[i]);
    }
    
    std::vector<string>::size_type size_gen = genres.size();
    for (std::vector<string>::size_type i = 0; i < size_gen; i++)
    {
